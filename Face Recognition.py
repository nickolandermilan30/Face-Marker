...
def detectAndDisplay(frame):
    ...
    for (x, y, w, h) in faces:
        ...
        # Draw stickman body below the face
        body_length = 2 * h
        body_start = (x + w//2, y + h)
        body_end = (x + w//2, y + h + body_length)
        frame = cv.line(frame, body_start, body_end, (0, 255, 0), 4)

        # -- In each face, detect eyes
        eyes = eyes_cascade.detectMultiScale(frame_gray[y:y+h, x:x+w])
